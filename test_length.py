...
check("over the cap, ▼ balances it (geometric mean)",
      abs(M.length_hours(10000.0, 500, "Action", COEFS, 0.4) - 4000 ** 0.5) < 1e-9)
check("over the cap with long ▼ stays capped",
      M.length_hours(5000.0, 500, "Action", COEFS, 3000.0) == M.LENGTH_CAP_H)
check("under the cap, ▼ is ignored",
      abs(M.length_hours(40.0, 500, "Action", COEFS, 0.1) - 40.0) < 1e-9)
...
check("guarded row carries raw figure + reason", len(LX) == 5 and LX[3] > 1000 and LX[0] < 1000 and LX[4] == "cap")
