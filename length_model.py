...
Two modes, one writer per file:
  ...
  python length_model.py         every playtime pass (chained in 2.3, or 3.4) -> length.json
      Length(h) = coef[g] × 10^( w·log10(▲) + (1−w)·log10(typical[g]) ),  w = min(n_up, K)/K,
                  capped at LENGTH_CAP_H (420 h); and when the uncapped figure passes the
                  cap, or the game is tagged Idler, it is blended down with ▼:
                  √(capped × coef·▼), never above the capped figure
...
# person plausibly plays. Order: clamp to the cap FIRST, then blend with ▼ — √(capped × coef·▼) —
# and never let the blend raise the figure. A ▼ balance for everything past 100 h was tried and
# dropped: measured against HLTB it cut well-known long games far too short (Baldur's Gate 3
# 137 → 74 h vs HLTB extra 117, Kenshi 107 → 38 vs 134), because in deep games the players who
# quit early are exactly the detractors. The fan-to-detractor gap did not separate idling from
# depth either (Kenshi 7.7×). See LENGTH_MODEL.md §5.
LENGTH_CAP_H = 420.0
...
def length_hours(up_h, n_up, genre, coefs, down_h=None, balance=False):
    ...
    raw = coef * up_eff
    h = min(raw, cap)
    if (balance or raw > cap) and down_h:   # ▼ balance — see LENGTH_CAP_H / BALANCE_TAGS
...
def main(argv):
    ...
    # The ceiling is this file's policy, not a fitted value: a
    # length_coefs.json written before a change must not hold an old cap in place.
    coefs = dict(load(COEF_FILE), cap_h=LENGTH_CAP_H)
